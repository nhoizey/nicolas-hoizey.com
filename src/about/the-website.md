...
# With <del>Jekyll</del> Eleventy, static is fantastic

The whole site is generated by Eleventy[^jekyll] from documents written in [Markdown](http://en.wikipedia.org/wiki/Markdown) syntax. It was last generated on&nbsp;{{ build.timestamp | date("Do MMMM YYYY") }}.

[^jekyll]: It was previously generated by [Jekyll](http://jekyllrb.com/), another famous Static Site Generator.

In order to migrate the hundreds of contents written since 2001 in specific [SPIP](https://spip.net)[^spip] syntax, I have developed a [SPIP plugin → Markdown](https://github.com/nhoizey/spip2markdown) which transforms as much as possible this specific syntax into Markdown syntax, including with the YAML Front Matter.

[^spip]: SPIP is an excellent free French content management tool, particularly relevant for editorial content.

## Some plugins

[Many](https://jekyllrb.com/docs/plugins/#available-plugins) [plugins](https://www.jekyll-plugins.com/) exist in the Jekyll ecosystem to enrich the basic solution, but as everywhere, not all of them work well, some impose contradictory operating modes, or are simply not sufficiently configurable. Be careful, most of these plugins prevent the automatic generation of [GitHub Pages](https://help.github.com/articles/using-jekyll-with-pages/) pages.

The images benefit from my [Jekyll Cloudinary] plugin (https://nhoizey.github.io/jekyll-cloudinary/) to use the SaaS [Cloudinary] service (https://cloudinary.com/invites/lpov9zyyucivvxsnalc5/sgyyc0j14k6p0sbt51nw) in order to generate the different versions necessary for Responsive Web Design, while optimizing performance.

Still concerning images, but also any other file that I would like to associate to a post (PDF or other), my plugin [jekyll-postfiles](https://nhoizey.github.io/jekyll-postfiles/) allows to drop them in the same place as the Markdown file, which greatly facilitates publication.

The tags are natively supported by Jekyll, but greatly improved by the [Jekyll Tagging](https://github.com/pattex/jekyll-tagging) plugin, which generates the individual pages for each tag, as well as the [tag cloud](https://nicolas-hoizey.com/tags/).

The archive pages by year and month are generated by the [Jekyll Archives](https://github.com/jekyll/jekyll-archives)[^arch-tags] plugin.

[^arch-tags]: This plugin can also generate category and tag archives, but I don't use the first ones, and the second ones are already generated - better - by the dedicated plugin.

Whenever possible, external resources are loaded asynchronously so as not to penalize your navigation, or even only on demand, as is the case for Youtube videos thanks to the [Jekyll Youtube Lazyloading] plugin (https://github.com/erossignon/jekyll-youtube-lazyloading) which displays only one image while waiting for the visitor to click to actually load the video.

The tweets are uploaded using the [Lazy Tweet Embeding] plugin (https://github.com/takuti/jekyll-lazy-tweet-embedding) which allows you to just write the URL of the tweet on one line, it couldn't be simpler.

This plugin inspired me to create a similar plugin for Youtube or Vimeo videos, currently under development.

Images to illustrate tag pages and *opengraph* meta pages use the [Jekyll File Exists] plugin (https://github.com/michaelx/jekyll_file_exists).

The [Webmentions](https://www.w3.org/TR/webmention/), finally, are collected by the [Bridgy](https://brid.gy/) and [webmention.io](https://webmention.io/) services, and are integrated to the site by the [jekyll-webmention_io](https://github.com/aarongustafson/jekyll-webmention_io/) plugin by [Aaron Gustafson](https://twitter.com/aarongustafson).

# The webperf

Web site performance has been one of my dadas for many years, being above all [frustrated as a simple visitor](https://twitter.com/nhoizey/status/562873571073355776/photo/1) by sites that are slower and heavier than the others.
...
For this, several recipes were combined:

For basic operations such as concatenation and CSS and JS minification, plus adding a hash to force a long cache to the browser, the [more





c'est à dire que les pages et leurs contenus venant du même domaine sont envoyées tel quel par le serveur, sans aucune création à la volée[^perf].
